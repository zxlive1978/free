...
/* Отладка ошибок */
	...
	//read all
	if ( $whatdo == 'read'){
		$query="SELECT * FROM ".$name_base.".".$table." WHERE (".$x_id_top."<=".(int)$start_time." AND ".$x_id_bot."<=".(int)$end_time.") OR (".$x_id_top.">=".(int)$start_time." AND ".$x_id_bot.">=".(int)$end_time.") OR (".$x_id_top."<=".(int)$start_time." AND ".$x_id_bot.">=".(int)$end_time.") OR (".$x_id_top.">=".(int)$start_time." AND ".$x_id_bot."<=".(int)$end_time.") ORDER BY type, top, numb;";
		$result=mysqli_query($dbc,$query) or die(mysqli_sqlstate($dbc));
		$comment = array();
